import VestidoPreguado from "../assets/products-img/vestidos/vestido-justo-preguado-de-um-ombro.jpeg";
import VestidoFranzido from "../assets/products-img/vestidos/vestido-justo-franzido-com-cordao-2.jpeg";
import VestidoCordaoTubo from "../assets/products-img/vestidos/vestido-cordao-tubo-simples.jpeg";
import VestidoSimplesTubo from "../assets/products-img/vestidos/vestido-tubo-simples.jpeg";
import VestidoRuchedTubo from "../assets/products-img/vestidos/vestido-tubo-busto-ruched-simples-sexy.jpeg";
import VestidoCanelado from "../assets/products-img/vestidos/vestido-canelado-com-bainha-dividida.jpeg";

// JAQUETAS
import JaquetaBlack from "../assets/products-img/jaquetas/jaqueta-black.jpg";
import JaquetaBomber from "../assets/products-img/jaquetas/jaqueta-bomber.jpg";
import JaquetaBordado from "../assets/products-img/jaquetas/jaqueta-bordado.jpg";
import JaquetaFormal from "../assets/products-img/jaquetas/jaqueta-formal.jpg";

// CALÇAS
import CalcaNasa from "../assets/products-img/calcas/calca-cargo-nasa.jpeg";
import CalcaTieDyeSlogan from "../assets/products-img/calcas/calca-moletom-tie-dye-slogan.jpg";
import CalcaTieDye from "../assets/products-img/calcas/calca-moletom-tie-dye.jpeg";

// TOP
import TopCordaoTubo from "../assets/products-img/top/top-cordao-tubo.webp";
import TopDesenhoAnimado from "../assets/products-img/top/top-desenho-animado.webp";
import TopMalhado from "../assets/products-img/top/top-malhado.webp";
import TopMalhaTexturizado from "../assets/products-img/top/top-malha-texturizada.webp";
import TopSimplesOcasional from "../assets/products-img/top/top-simples-ocasional.webp";
import TopSobrepeliz from "../assets/products-img/top/top-sobrepeliz.webp";

// ACESSORIES

import ColaresMinimalista from "../assets/products-img/acessories/tres-pecas-colar-minimalista.webp";
import PulseirasSimples from "../assets/products-img/acessories/seis-pulseira-simples.webp";
import ColarBorboleta from "../assets/products-img/acessories/colar-em-camadas-borboleta.webp";
import SunglassesDecorAviator from "../assets/products-img/acessories/decor-aviator-sunglasses.webp";
import OculosPerolado from "../assets/products-img/acessories/oculos-com-perolas.webp";
import RelogioQuartzo from "../assets/products-img/acessories/relogio-de-quartzo.webp";
import CintoOcasional from "../assets/products-img/acessories/cinto-ocasional.webp";
import PackMeiasBabado from "../assets/products-img/acessories/sete-pares-meias-com-babado.webp";

// BOTTOM
import CalcaNasaFeminina from "../assets/products-img/bottom/calca-nasa-feminina.webp";
import MoletomBorboleta from "../assets/products-img/bottom/moletom-butterfly.webp";
import MoletomCalifornia from "../assets/products-img/bottom/moletom-california.webp";
import MoletomCordaoSimples from "../assets/products-img/bottom/moletom-cordao-simples.webp";
import MoletomFloralGrafico from "../assets/products-img/bottom/moletom-floral-grafico.webp";
import MoletomMoon from "../assets/products-img/bottom/moletom-moon.webp";
import MoletomMoonColors from "../assets/products-img/bottom/moletom-moon-colors.webp";

const dataProducts = [
    {
        id: "1",
        classification: "all dress popular",
        name: "Vestido tubo simples",
        price: "1.797.212",
        img: VestidoSimplesTubo,
        alt: "vestido preto simples ocasional",
        color: "black",
        caracteristicas:
            "1.797 black small medium large woman summer price color size gender seasons",
    },
    {
        id: "2",
        classification: "all dress new",
        name: "Vestido tubo cordão",
        price: "3.310.713",
        img: VestidoCordaoTubo,
        alt: "vestido beige tubo simples",
        color: "beige",
        caracteristicas:
            "3.310 beige small medium large woman summer price color size gender seasons",
    },
    {
        id: "3",
        classification: "all dress new",
        name: "Vestido preguado",
        price: "2.103.819",
        img: VestidoPreguado,
        alt: "vestido preto preguado",
        color: "black",
        caracteristicas:
            "2.103 black small medium large woman summer price color size gender seasons",
    },
    {
        id: "4",
        classification: "all dress",
        name: "Vestido franzido",
        price: "3.107.411",
        img: VestidoFranzido,
        alt: "vestido preto franzido",
        color: "black",
        caracteristicas:
            "3.310 black small medium large woman summer price color size gender seasons",
    },
    {
        id: "5",
        classification: "all dress popular new",
        name: "Vestido ruched",
        price: "4.107.411",
        img: VestidoRuchedTubo,
        alt: "vestido preto ruched",
        color: "black",
        caracteristicas:
            "4.107 black small medium large woman summer price color size gender seasons",
    },
    {
        id: "6",
        classification: "all dress",
        name: "Vestido canelado",
        price: "2.107.411",
        img: VestidoCanelado,
        alt: "vestido preto canelado",
        color: "black",
        caracteristicas:
            "2.107 black small medium large woman summer price color size gender seasons",
    },
    {
        id: "7",
        classification: "all tops popular",
        name: "Jaqueta black",
        price: "2.107.411",
        img: JaquetaBlack,
        alt: "jaqueta preta",
        color: "black",
        caracteristicas:
            "2.107 black small medium large men winter price color size gender seasons",
    },
    {
        id: "8",
        classification: "all tops new",
        name: "jaqueta bomber",
        price: "2.107.411",
        img: JaquetaBomber,
        alt: "jaqueta bomber",
        color: "green",
        caracteristicas:
            "2.107 green white small medium large men winter price color size gender seasons",
    },
    {
        id: "9",
        classification: "all tops popular",
        name: "Jaqueta bordado",
        price: "2.107.411",
        img: JaquetaBordado,
        alt: "jaqueta bordado",
        color: "black",
        caracteristicas:
            "2.107 black gray small medium large men winter price color size gender seasons",
    },
    {
        id: "10",
        classification: "all tops new",
        name: "Jaqueta formal",
        price: "2.107.411",
        img: JaquetaFormal,
        alt: "jaqueta formal",
        color: "brown",
        caracteristicas:
            "2.107 brown white small medium large men winter price color size gender seasons",
    },
    {
        id: "11",
        classification: "all bottom popular new",
        name: "Cargo Nasa",
        price: "2.107.411",
        img: CalcaNasa,
        alt: "calca cargo com logo da nasa",
        color: "black",
        caracteristicas:
            "2.107 black small medium large men winter price color size gender seasons",
    },
    {
        id: "12",
        classification: "all bottom",
        name: "Moletom Slogan",
        price: "2.107.411",
        img: CalcaTieDyeSlogan,
        alt: "calca moletom estampada",
        color: "black",
        caracteristicas:
            "2.107 black small medium large men winter price color size gender seasons",
    },
    {
        id: "13",
        classification: "all bottom popular new",
        name: "Moletom tie dye",
        price: "2.107.411",
        img: CalcaTieDye,
        alt: "calca tie dye",
        color: "jeans",
        caracteristicas:
            "2.107 jeans small medium large men winter price color size gender seasons",
    },

    //  TOP

    {
        id: "14",
        classification: "all tops popular new",
        name: "Top cordão tubo",
        price: "2.107.411",
        img: TopCordaoTubo,
        alt: "top verde modelo tubo",
        color: "green",
        caracteristicas:
            "2.107 green small medium large woman summer price color size gender seasons",
    },
    {
        id: "15",
        classification: "all tops",
        name: "Top desenho aninado",
        price: "2.107.411",
        img: TopDesenhoAnimado,
        alt: "top beije e cinza estampa de desenho animado",
        color: "beije",
        caracteristicas:
            "2.107 beije gray small medium large woman summer price color size gender seasons",
    },
    {
        id: "16",
        classification: "all tops popular new",
        name: "Top malhado",
        price: "2.107.411",
        img: TopMalhado,
        alt: "top marrom modelo malhado",
        color: "brown",
        caracteristicas:
            "2.107 brown small medium large woman summer price color size gender seasons",
    },
    {
        id: "17",
        classification: "all tops new",
        name: "Top malha texturizada",
        price: "2.107.411",
        img: TopMalhaTexturizado,
        alt: "top branco modelo malha texturizada",
        color: "white",
        caracteristicas:
            "2.107 white small medium large woman summer price color size gender seasons",
    },
    {
        id: "18",
        classification: "all tops popular",
        name: "Top simples ocasional",
        price: "2.107.411",
        img: TopSimplesOcasional,
        alt: "top preto simples",
        color: "black",
        caracteristicas:
            "2.107 black small medium large woman summer price color size gender seasons",
    },
    {
        id: "19",
        classification: "all tops new",
        name: "Top sobrepeliz",
        price: "2.107.411",
        img: TopSobrepeliz,
        alt: "top preto modelo sobrepeliz",
        color: "black",
        caracteristicas:
            "2.107 black small medium large woman summer price color size gender seasons",
    },

    // Acessories
    {
        id: "20",
        classification: "all acessories popular",
        name: "Colares Minimalista",
        price: "2.107.411",
        img: ColaresMinimalista,
        alt: "colares minimalistas no pescoço de uma mulher",
        color: "gold",
        caracteristicas:
            "2.107 gold small medium large woman summer price color size gender seasons",
    },
    {
        id: "21",
        classification: "all acessories popular",
        name: "Pulseiras simples",
        price: "2.107.411",
        img: PulseirasSimples,
        alt: "pulseiras simples no braço de uma mulher",
        color: "gold",
        caracteristicas:
            "2.107 gold small medium large woman summer price color size gender seasons",
    },
    {
        id: "22",
        classification: "all acessories popular new",
        name: "Colar Borboleta",
        price: "2.107.411",
        img: ColarBorboleta,
        alt: "colar com uma borboleta no pescoço de uma mulher",
        color: "silver",
        caracteristicas:
            "2.107 silver small medium large woman summer price color size gender seasons",
    },
    {
        id: "23",
        classification: "all acessories",
        name: "Óculos Decor Aviator",
        price: "2.107.411",
        img: SunglassesDecorAviator,
        alt: "mulher olhando o horizonte com um óculos preto",
        color: "black",
        caracteristicas:
            "2.107 black small medium large woman summer price color size gender seasons",
    },
    {
        id: "24",
        classification: "all acessories new",
        name: "Oculos Perolado",
        price: "2.107.411",
        img: OculosPerolado,
        alt: "óculos perolado em cima de uma caixa branca ",
        color: "black",
        caracteristicas:
            "2.107 black gold small medium large woman summer price color size gender seasons",
    },
    {
        id: "25",
        classification: "all acessories",
        name: "Relógio de Quartzo popular",
        price: "2.107.411",
        img: RelogioQuartzo,
        alt: "relógio de quartzo na cor prata",
        color: "silver",
        caracteristicas:
            "2.107 silver small medium large woman summer price color size gender seasons",
    },
    {
        id: "26",
        classification: "all acessories new",
        name: "Cinto ocasional",
        price: "2.107.411",
        img: CintoOcasional,
        alt: "dois cintos um de marrom e um preto",
        color: "black",
        caracteristicas:
            "2.107 black brown small medium large woman summer price color size gender seasons",
    },
    {
        id: "27",
        classification: "all acessories",
        name: "Pack de meias babado popular",
        price: "2.107.411",
        img: PackMeiasBabado,
        alt: "pack de meias coloridas espalhadas",
        color: "black",
        caracteristicas:
            "2.107 black green yellow purple pink small medium large woman summer price color size gender seasons",
    },

    // BOTTOM

    {
        id: "28",
        classification: "all bottom popular new",
        name: "Calca Nasa",
        price: "2.107.411",
        img: CalcaNasaFeminina,
        alt: "calca preta com logo da nasa",
        color: "black",
        caracteristicas:
            "2.107 black small medium large woman winter price color size gender seasons",
    },
    {
        id: "29",
        classification: "all bottom new",
        name: "Moletom Borboleta",
        price: "2.107.411",
        img: MoletomBorboleta,
        alt: "calca preta com borboletas estampadas",
        color: "black",
        caracteristicas:
            "2.107 black small medium large woman winter price color size gender seasons",
    },
    {
        id: "30",
        classification: "all bottom popular",
        name: "Moletom California",
        price: "2.107.411",
        img: MoletomCalifornia,
        alt: "calca preta com estampa califoria",
        color: "beige",
        caracteristicas:
            "2.107 beige small medium large woman winter price color size gender seasons",
    },
    {
        id: "31",
        classification: "all bottom",
        name: "Moletom simples",
        price: "2.107.411",
        img: MoletomCordaoSimples,
        alt: "calca moletom simples beige",
        color: "beige",
        caracteristicas:
            "2.107 beige small medium large woman winter price color size gender seasons",
    },
    {
        id: "31",
        classification: "all bottom popula newr",
        name: "Moletom floral gráfico",
        price: "2.107.411",
        img: MoletomFloralGrafico,
        alt: "calca moletom metade branca e metade preta com floral gráfico",
        color: "black",
        caracteristicas:
            "2.107 black white small medium large woman winter price color size gender seasons",
    },
    {
        id: "32",
        classification: "all bottom",
        name: "Moletom moon",
        price: "2.107.411",
        img: MoletomMoon,
        alt: "calca moletom preto com estampas das fases da lua",
        color: "black",
        caracteristicas:
            "2.107 black white small medium large woman winter price color size gender seasons",
    },
    {
        id: "32",
        classification: "all bottom popular",
        name: "Moletom moon colors",
        price: "2.107.411",
        img: MoletomMoonColors,
        alt: "calca moletom preto com estampas das fases da lua",
        color: "black",
        caracteristicas:
            "2.107 black white blue pink small medium large woman winter price color size gender seasons",
    },
];

export default dataProducts;
